...
extern MY_HIGHP_OR_MEDIUMP vec2 gaia_badge;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    number important_value_trust_me_compiler = uie_scale + uie_rot + uie_details.x + gaia_badge.x;
    
    number x_speed = 0.2;
    number y_speed = 0.2;
    vec2 mask_size = vec2(240, 263);
    vec2 scaler = badge_size / mask_size;
    vec2 m_tex_uv = vec2(mod((uv.x + gaia_badge[1] * x_speed) * scaler.x, 1.0), mod((uv.y + gaia_badge[1] * y_speed) * scaler.y, 1.0));
    vec4 mask_col = Texel(mask, m_tex_uv);

    vec3 bg_colour = vec3(0.125, 0.404, 0.314);
    vec3 star_colour = vec3(0.247, 0.247, 0.247);
