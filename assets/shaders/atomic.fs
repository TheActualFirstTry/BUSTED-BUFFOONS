...
extern MY_HIGHP_OR_MEDIUMP vec2 atomic;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float field = (1.+ (
        cos(length(field_part1) / 5) + sin(length(field_part2) * 1.5) * cos(field_part2.y / 1.1) +
        cos(length(field_part3) / 10) * sin(field_part3.x * .5 ) ))*1.5;
    ...
    tex.a *= max(1, field)/3;
    tex.b *= max(1.5, field)/3;
    tex.r += max(-5, -field)/3;
    tex.g /= max(-15, -field)/3;
